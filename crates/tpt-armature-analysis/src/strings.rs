...
//! surfaced in the CLI (`tpt-armature strings`) and the GUI strings panel, and are
//! handy triage leads (format strings, URLs, magic numbers, jump-table offsets).

use tpt_armature_formats::MemoryMap;
...
pub fn extract_constants(instructions: &[tpt_armature_ir::Instruction], max: usize) -> Vec<u64> {
    use std::collections::BTreeSet;
    let mut set = BTreeSet::new();
    for ins in instructions {
        for op in &ins.operands {
            if let tpt_armature_ir::Operand::Imm(v) = op {
...
mod tests {
    use super::*;
    use tpt_armature_formats::{MemoryMap, Section};

    fn map_with(bytes: &[u8]) -> MemoryMap {
        MemoryMap {
            format: tpt_armature_formats::BinaryFormat::Unknown,
            arch: tpt_armature_formats::Architecture::X86_64,
            base_address: 0x1000,
            entry_point: 0,
    ...
    fn extracts_utf16_run() {
        ...
        assert!(strings
            .iter()
            .any(|s| s.text == "Hi" && s.kind == StringKind::Utf16));
    }
