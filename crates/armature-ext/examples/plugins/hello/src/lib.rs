...
//!
//! Then load the produced `target/wasm32-unknown-unknown/release/armature-hello-plugin.wasm`
//! from the host (`armature-ext` `wasm` feature) via `PluginHost::load` + `run`.
//!
//! The host exposes the `armature` module with three imports:
...
#[link(wasm_import_module = "armature")]
extern "C" {
...
#[no_mangle]
pub extern "C" fn armature_run() {
    let count = unsafe { get_instruction_count() };
    emit("hello from armature plugin; instructions = ");
